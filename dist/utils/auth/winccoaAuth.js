...
import winccoa from "../globalWinccoaManager.js";
async function setManagerUserByName(user) {
    const wccoaScript = new WinccoaCtrlScript(winccoa, `int obtainUserId(string user)
        {
            return getUserId(user);
        }`);
    let userId = await wccoaScript.start('obtainUserId', [user], [WinccoaCtrlType.string]);
    let userSetted = winccoa.setUserId(userId);
